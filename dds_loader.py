...
class DDSSurface:
  def __init__(self, width, height, pitch, size, offset):
    self.width = width
    self.height = height
    self.pitch = pitch
    self.size = size
  ...
  def __str__(self):
    return "Width: {0} Height: {1} Pitch: {2} Size: {3} Offset: {4}".format(self.width, self.height, self.pitch, self.size, self.offset)
...
class DDSTexture:
  ...
  def _compute_type(self):
    ...
    # We either have a single texture or a texture array ( 2D )
      ...
      if self.header.ddspf.dwFourCC == DDSEnums.DX10_CC and \
    ...
    with open(filename, "rb") as file_stream:
      ...
      for i in range(elements):
        ...
        for mipmap in range(self.header.dwMipMapCount):
          ...
          self.surfaces.append(DDSSurface(next_width, next_height, pitch, next_size, offset))
          offset += next_size
